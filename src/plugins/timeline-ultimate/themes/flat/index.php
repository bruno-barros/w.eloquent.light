...
function timeline_um_body_flat($post_id)
{
	...
	$args_default = array(
		'meta_query' => array(
			...
			array(
				'key'     => 'to_client',
				'value'   => '"'.\Intra\Entities\Clients\Client::id().'"',
				'compare' => 'LIKE',
			),array(
				'key'     => 'to_client',
				'compare' => 'NOT EXISTS',
	...
	{
		$wp_query = new WP_Query( array_merge($args_default, array(
	...
	if ($wp_query->have_posts()) :
		...
		while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			$timeline_um_body .= '<div class="timeline_um-title" style="color:' . $timeline_um_items_title_color . ';font-size:' . $timeline_um_items_title_font_size . '"><a href="'.get_permalink().'">' . get_the_title() . '</a>
			</div>';
			...
			{
				$timeline_um_body .= '' . get_the_date();
			}

			if ($timeline_um_items_author == 'yes')
			{
				$timeline_um_body .= ' ' . get_the_author();
			}

			if ($timeline_um_items_categories == 'yes')
			{
				$timeline_um_body .= ' Categoria: ' . trim($category_output, $separator);
			}
	...
	$timeline_um_body .= '<div timeline_id="' . $post_id . '" themes="' . $timeline_um_themes . '" offset="' . $timeline_um_total_items . '" per_page="' . $timeline_um_total_items . '" class="load-more"><span>Carregar mais</span></div>';
...
function timeline_um_body_ajax_flat()
{
	...
	if ($wp_query->have_posts()) :
		...
		while ($wp_query->have_posts()) : $wp_query->the_post();
			...
			if ($timeline_um_items_author == 'yes')
			{
				$timeline_um_body .= ' por: ' . get_the_author();
			}
		...
		jQuery(document).ready(function($)
			{
				$(".load-more").html("<span>Nada encontrado</span>");
