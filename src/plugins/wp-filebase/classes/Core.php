...
class WPFB_Core
{
    ...
    {
        return is_multisite() ? add_query_arg('blog_id', get_current_blog_id(), WPFB_PLUGIN_URI . $url)
            : (WPFB_PLUGIN_URI . $url);
    }
    ...
    static function InitClass()
    {
        ...
        add_filter('the_content', array(
            __CLASS__,
            'ContentFilter'
        ), 10); // must be lower than 11 (before do_shortcode) and after wpautop (>9)
        ...
        wp_register_script('jquery-treeview', WPFB_PLUGIN_URI
                                              . 'extras/jquery/treeview/jquery.treeview.js', array('jquery'), WPFB_VERSION);
        wp_register_script('jquery-treeview-edit', WPFB_PLUGIN_URI
                                                   . 'extras/jquery/treeview/jquery.treeview.edit.js', array('jquery-treeview'), WPFB_VERSION);
        wp_register_script('jquery-treeview-async', WPFB_PLUGIN_URI
                                                    . 'extras/jquery/treeview/jquery.treeview.async.js', array('jquery-treeview-edit'), WPFB_VERSION);
        wp_register_style('jquery-treeview', WPFB_PLUGIN_URI
                                             . 'extras/jquery/treeview/jquery.treeview.css', array(), WPFB_VERSION);

        // DataTables
        wp_register_script('jquery-dataTables', WPFB_PLUGIN_URI
                                                . 'extras/jquery/dataTables/js/jquery.dataTables.min.js', array('jquery'), WPFB_VERSION);
        wp_register_style('jquery-dataTables', WPFB_PLUGIN_URI
                                               . 'extras/jquery/dataTables/css/jquery.dataTables.css', array(), WPFB_VERSION);

        wp_register_script('jquery-dataTables-columnFilter', WPFB_PLUGIN_URI
                                                             . 'extras/jquery/dataTables/js/jquery.dataTables.columnFilter.js', array('jquery-dataTables'), WPFB_VERSION);

        wp_register_script(WPFB, WPFB_PLUGIN_URI
                                 . 'js/common.js', array('jquery'), WPFB_VERSION); // cond loading (see Footer)
        ...
        if (empty(WPFB_Core::$settings->disable_css))
        {
            ...
            {
                $upload_path = path_is_absolute(WPFB_Core::$settings->upload_path) ? ''
                    : WPFB_Core::$settings->upload_path;
                wp_enqueue_style(WPFB, WPFB_Core::PluginUrl("wp-filebase_css.php?rp=$upload_path"), array(), WPFB_VERSION, 'all');
            }
        }


        $wpfb_admin_page = (is_admin() && !empty($_GET['page']) && strpos($_GET['page'], 'wpfilebase_') !== false)
                           || defined('WPFB_EDITOR_PLUGIN');
        if ($wpfb_admin_page)
        {
        ...
        // live admin
        if (($wpfb_admin_page && @$_GET['page'] == 'wpfilebase_filebrowser')
            || ((WPFB_Core::CurUserCanCreateCat()
                 || WPFB_Core::CurUserCanUpload())
                && !is_admin())
        )
        {
            wp_enqueue_script(WPFB . '-live-admin', WPFB_PLUGIN_URI
                                                    . 'js/live-admin.js', array('jquery'), WPFB_VERSION);
            ...
            {
                wp_enqueue_script('jquery-contextmenu', WPFB_PLUGIN_URI
                                                        . 'extras/jquery/contextmenu/jquery.contextmenu.js', array('jquery'));
                wp_enqueue_style('jquery-contextmenu', WPFB_PLUGIN_URI
                                                       . 'extras/jquery/contextmenu/jquery.contextmenu.css', array(), WPFB_VERSION);
            }
    ...
    {
        return empty($name) ? (array)WPFB_Core::$settings
            : (isset(WPFB_Core::$settings->$name) ? WPFB_Core::$settings->$name : null);
    }
    ...
    static function GetPostId($query = null)
    {
        ...
        return ((!empty($query->post) && $query->post->ID > 0)
            ? $query->post->ID
            :
            (!empty($query->queried_object_id)
                ? $query->queried_object_id
                :
                (!empty($query->query['post_id'])
                    ? $query->query['post_id']
                    :
                    (!empty($query->query['page_id'])
                        ? $query->query['page_id']
                        :
                        0))));
    }
    ...
    static function ParseQuery(&$query)
    {
        ...
        if (!empty($_GET['wpfb_s']) || !empty($_GET['s']))
        {
            ...
            add_filter('the_excerpt', array(
                __CLASS__,
                'SearchExcerptFilter'
            ), 100); // must be lower than 11 (before do_shortcode) and after wpautop (>9)
        }
    ...
    static function DownloadRedirect()
    {
        ...
        else
        {
            ...
            if ($pos === 0)
            {
                ...
                {
                    wpfb_loadclass('File', 'Category');
                    $file = is_null($file = WPFB_File::GetByPath($filepath))
                        ? WPFB_File::GetByPath(urldecode($filepath)) : $file;
                }
            }
        }
    ...
    static function Footer()
    {
        ...
        {
            echo '<div id="wpfb-credits" name="wpfb-credits" style="'
                 . esc_attr(WPFB_Core::$settings->footer_credits_style) . '">';
    ...
    static function UploadDir()
    {
        ...
        return empty($upload_path) ? ($upload_path = path_join(ABSPATH, empty(WPFB_Core::$settings->upload_path)
            ? 'wp-content/uploads/filebase' : WPFB_Core::$settings->upload_path)) : $upload_path;
    }


    static function GetPostUrl($id)
    {
        return isset(self::$post_url_cache[$id]) ? self::$post_url_cache[$id]
            : (self::$post_url_cache[$id] = get_permalink($id));
    }
    ...
    static function EnqueueScripts()
    {
        ...
        if (!WPFB_Core::$settings->late_script_loading
            && ((!empty($wp_query->queried_object_id)
                 && $wp_query->queried_object_id == WPFB_Core::$settings->file_browser_post_id)
    ...
    static function PrintJS()
    {
        ...
        $context_menu =
            current_user_can('upload_files') && self::GetOpt('file_context_menu') && !defined('WPFB_EDITOR_PLUGIN')
            && !is_admin();
...
// gets custom template list or single if tag specified
    ...
    static function Cron()
    {
        ...
        {
            wpfb_call('Sync', 'Sync');
            update_option(WPFB_OPT_NAME . '_cron_sync_time', empty($_SERVER["REQUEST_TIME"]) ? time()
    ...
    {
        $custom_fields  = isset(WPFB_Core::$settings->custom_fields)
    ...
    {
        $path = empty($path) ? self::UploadDir()
            : (ABSPATH . '/' . trim(str_replace('\\', '/', str_replace('..', '', $path)), '/'));
    ...
    static function PluginsApiFilter($value, $action = null, $args = null)
    {
        ...
        return ($action === 'plugin_information'
                && (strpos($args->slug, "wp-filebase") === 0
                    || strpos($args->slug, "wpfb-") === 0))
